...
def mode_creature() : 
    ...
    ferme la page actif de jeu
    """
    pyautogui.keyDown("shift")#fermetuer de la fenetre dofus
    pyautogui.press("(")
...
def my_map_dofus(config : str )  :
    ...
    """
    map = ""
    image = pyautogui.screenshot(region = v.region_map )  # "data" is a height x width x 4 numpy array
    image = traitement_image_map(image)
    map = pytesseract.image_to_string(image, lang="eng", config=config)
    
    return filtre_my_map(map)
    
def changement_map(image, dep,pos):
    a = 1
    x,y = dep.x, dep.y
...
def travel(pos : str,t:float) : # effectue un voyage jusqu'a la map pos, t -> durée du voyage en s
    ...
    """
    print("DEBUT du voyage en dragodinde")
    time.sleep(3)
    travel = "/travel" + " " + pos
    ...
    pyautogui.click(v.region_PA[0],v.region_PA[1]-15)
    time.sleep(1)
    print("FIN du voyage en dragodinde")
...
def go_bank(my_pos:str ,region:str = "Amakna") :
    match region :
        ...
        case "Otomai" : 
            pos_banque = v.pos_banque_otomai
            x_entre_banque, y_entre_banque = v.x_entre_banque_otomai, v.y_entre_banque_otomai
            x_sortie_banque, y_sortie_banque = v.x_sortie_banque_otomai, v.y_sortie_banque_otomai
            x_coffre_guilde_ouvrir, y_coffre_guilde_ouvrir = v.x_coffre_guilde_otomai_ouvrir, v.y_coffre_guilde_otomai_ouvrir 
...
def enter_bank(my_pos:str, pos_bank : str, entre_bank_x, entre_bank_y):
    ...
    """
    travel(pos_bank,distance_map(pos_bank,my_pos)*10+20) # voyage jusqu'a la banque (compte le nb de map * 10 secondes )
...
def transfert_coffre_ressource(onglet_inventaire,nb) :
    match onglet_inventaire :
        case "ressource" :
            dofus_click(v.x_coffre_guilde_onglet_ressource, v.y_coffre_guilde_onglet_ressource,0.5,4)
            transfert_ressource(v.x_coffre_guilde_ressource, v.y_coffre_guilde_ressource,nb)
        case "potion" :
